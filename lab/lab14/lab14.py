...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if check.deposited or self.holder != check.payable_to:
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            new_edits, new_cost = helper_align(skeleton_idx+1, code_idx+1)
            possibilities.append((skel_char + new_edits, new_cost))
        # Insert
        new_edits, new_cost = helper_align(skeleton_idx, code_idx+1)
        new_edits = "+[" + code_char + "]" + new_edits
        possibilities.append((new_edits, new_cost + 1))
        # Delete
        new_edits, new_cost = helper_align(skeleton_idx+1, code_idx)
        new_edits = "-[" + skel_char + "]" + new_edits
        possibilities.append((new_edits, new_cost + 1))
