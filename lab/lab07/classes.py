...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        #add opponent card attack & defense to cards in your hand
        for card in player.deck.cards:
            card.attack += other_card.attack
            card.defense += other_card.defense

        #find indexes to delete
        opponentDeckCopy = opponent.deck.cards[:]
        deleteIndex = [i for i in range(orig_opponent_deck_length) if opponentDeckCopy[i].attack == other_card.attack or opponentDeckCopy[i].defense == other_card.defense]
        opponent.deck.cards = [opponentDeckCopy[i] for i in range(orig_opponent_deck_length) if i not in deleteIndex]
