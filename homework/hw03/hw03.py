...
def balanced(m):
    ...
    if is_planet(m):
        ...
        torqueLeft = length(left(m)) * total_weight(end(left(m)))
        torqueRight = length(right(m)) * total_weight(end(right(m)))
...
def preorder(t):
    ...
    # add current label to preordering list
    # recurse down each child from leftmost to rightmost. What should we do with the recursive result there?
    # return preordering
